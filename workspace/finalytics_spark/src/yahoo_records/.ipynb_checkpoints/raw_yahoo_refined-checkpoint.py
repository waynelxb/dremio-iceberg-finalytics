...
class RawYahoo:
    ...
    def get_raw_eod_quote_consolidated(self) -> pd.DataFrame:
        """
        Fetch and consolidate EOD records for grouped symbols.
    
        Args:
            api (str): The API to use ("yahooquery" or "yfinance").
            self.grouped_symbol_list (list): List of tuples containing (group_id, group_start_date, symbol).
    
        Returns:
            pd.DataFrame: Consolidated DataFrame of EOD records.
        """
        try:
            if len(self.grouped_symbol_list)>0:
                ...
                for (group_id, group_start_date), group_symbols in grouped_symbols.items():
                    ...
                    hist_group_panda_df = self.get_raw_eod_quote(group_symbols, group_start_date)
    ...
    def get_raw_market_quote(self, symbol_list: list) -> pd.DataFrame:
        """
    ...
    def get_raw_market_quote_consolidated(self) -> pd.DataFrame:
        """
        Fetch and consolidate EOD records for grouped symbols.
    
        Args:
            grouped_symbol_start_date_list (list): List of tuples containing (group_id, group_start_date, symbol).
        ...
        try:
            ...
            for group_id, group_symbols in grouped_symbols.items():
                print(f"Processing group {group_id}/{largest_group_id} with {len(group_symbols)} symbols...")
                group_panda_df = self.get_raw_market_quote(group_symbols)
