...
def get_yahoo_raw_eod_records(api: str, symbol_list: list, start_date: date) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if api == "yahooquery":
            ...
            hist_data = yf.download(
                ...
                interval="1d",
                group_by="ticker",  # Keep data grouped by ticker
                auto_adjust=False,  # Keep original values without adjustment
            )
            # Reshape data for multiple tickers
            hist_data = hist_data.stack(level=0).reset_index()
            hist_data.rename(columns={"Date": "date", "Ticker": "symbol", "Open": "open", 
...
def get_yahoo_raw_eod_records_consolidated(api: str, group_date_symbol_list: list) -> pd.DataFrame:
    """
    ...
    try:
        if len(group_date_symbol_list)>0:
            ...
            for (group_id, group_start_date), group_symbols in grouped_symbols.items():
                ...
                hist_group_panda_df = get_yahoo_raw_eod_records(api, group_symbols, group_start_date)
