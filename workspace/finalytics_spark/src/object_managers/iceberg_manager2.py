...
class IcebergManager:
    ...
    def create_spark_df(self, schema, tuple_list):
        spark_df = self.spark_session.createDataFrame(tuple_list, schema=schema)
        

    def insert_into_iceberg_table(self, source_spark_df, target_iceberg_table, mode):
        try: 
            # Create namespace if not exists            
            self.spark_session.sql("CREATE NAMESPACE IF NOT EXISTS nessie.raw;")  

            # Check if the Iceberg table exists, if not, create it
            if self.spark_session.catalog.tableExists(target_iceberg_table):
